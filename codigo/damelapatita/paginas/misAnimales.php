                    ...
                    <div class="row">
                        ...
                        <div class="col-md-9" id="barrica">
                            ...
                            <div class="col-md-12">
                                ...
                                <table class="table table-bordered table-hover tablaAdopcion">
                                <?php
                                $stmt = verAnunciosPropios(2, $_SESSION['username']);
                                $stmt->execute();

                                while($row = $stmt->fetch()){
                                  echo '<tr class="warning">';
                                  echo '<td class="col-xs-3 col-sm-4">' . $row['nombre'] . ', ' . cadenaEdad($row['edad']) . '</td>';
                                  echo '<td class="col-xs-3 col-sm-3 modificar"><a href="http://localhost/damelapatita/paginas/modificarAnimal.php?anuncio=' . $row['id'] . '"><span class="glyphicon glyphicon-pencil"></span> Editar</a></td>';
                                  echo '<td class="col-xs-3 col-sm-3 borrar"><a href="http://localhost/damelapatita/llamadasPHP/eliminarAnuncio.php?anuncio=' . $row['id'] . '"><span class="glyphicon glyphicon-remove"></span> Borrar</a></td>';
                                  echo '<td class="col-xs-3 col-sm-4 solucionar"><a href="http://localhost/damelapatita/llamadasPHP/solucionarAnuncio.php?anuncio=' . $row['id'] . '"><span class="glyphicon glyphicon-heart"></span> Solucionado</a></td>';
                                  echo '</tr>';
                                } // Fin while
                                ?>
                                </table>
                            ...
                            <div class="col-md-12">
                                ...
                                <table class="table table-bordered table-hover tablaAdopcion">
                                <?php
                                $stmt = verAnunciosPropios(3, $_SESSION['username']);
                                $stmt->execute();
                            ...
                            <div class="col-md-12">
                                ...
                                <table class="table table-bordered table-hover tablaAdopcion">
                                <?php
                                $stmt = verAnunciosPropios(4, $_SESSION['username']);
                                $stmt->execute();

                                while($row = $stmt->fetch()){
                                  echo '<tr class="warning">';
                                  echo '<td class="col-xs-3 col-sm-4">Encontrado en ' . $row['calle'] . ', ' . $row['localidad'] . '. Provincia de ' . $row['provincia'] . '</td>';
                                  echo '<td class="col-xs-3 col-sm-3 modificar"><a href="http://localhost/damelapatita/paginas/modificarAnimal.php?anuncio=' . $row['id'] . '"><span class="glyphicon glyphicon-pencil"></span> Editar</a></td>';
                                  echo '<td class="col-xs-3 col-sm-3 borrar"><a href="http://localhost/damelapatita/llamadasPHP/eliminarAnuncio.php?anuncio=' . $row['id'] . '"><span class="glyphicon glyphicon-remove"></span> Borrar</a></td>';
                                  echo '<td class="col-xs-3 col-sm-4 solucionar"><a href="http://localhost/damelapatita/llamadasPHP/solucionarAnuncio.php?anuncio=' . $row['id'] . '"><span class="glyphicon glyphicon-heart"></span> Solucionado</a></td>';
                                  echo '</tr>';
